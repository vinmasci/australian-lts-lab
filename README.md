...
Selecting a mapped road opens its contribution controls. A contribution may contain an LTS vote, a rideability rating, or both; at least one rating is required, but neither scale is mandatory when submitting the other. Contributors sign in with an existing AusBUG Apple, Google or email/password account. Their verified Firebase UID identifies one editable contribution per account and segment. Their display name, reasoning and observations are shown publicly on the selected segment; email addresses and account identifiers remain private.

LTS 1.5 is the cyan class for a trafficable road above 30 km/h with very little motor traffic. Community results of LTS 1, 1.5 or 2 become that value when published. An LTS 3 or 4 community result is averaged with the source score and rounded up: for example, source LTS 2 plus an LTS 3 result becomes LTS 3.
...
Votes do not alter the underlying OSM-derived source network. Each signed-in contribution immediately recalculates the community result and updates the separate published override layer. Reconciled published overrides are incorporated into the isolated LTS routing graph during its weekly Monday rebuild; LTS 1.5 receives a routing cost between LTS 1 and LTS 2. The server verifies the contributor's Firebase ID token, keys their vote to the verified UID and applies account and connection rate limits. The protected `/ltsmap/review` page (also `/review`) remains available to allowlisted reviewers for audits, corrections and removal; account creation remains with the main AusBUG app.

Raw votes and private account identifiers are stored server-side under `ltsSegments/{segment}/votes`; every accepted submission also creates a compact, append-only audit record in `ltsContributionEvents`, preserving whether it was a first vote or an edit and which fields changed without duplicating email addresses or map geometry. Publications and reviewer corrections have append-only audit records in `ltsReviewDecisions`, while current results are stored in `ltsApprovals`. Segment summaries expose only the contributor display name, ratings, reasoning and observation. Email addresses, Firebase UIDs, voter hashes and reviewer identity are never returned publicly. The sanitised `ltsPublishedSegments` collection continues to feed the statewide overlay without comments. Tied vote totals lean toward the higher-stress value so that uncertainty is not hidden.
