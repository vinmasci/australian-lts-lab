...
Raw votes and private account identifiers are stored server-side under `ltsSegments/{segment}/votes`; publications and reviewer corrections have append-only audit records in `ltsReviewDecisions`, while current results are stored in `ltsApprovals`. Segment summaries expose only the contributor display name, ratings, reasoning and observation. Email addresses, Firebase UIDs, voter hashes and reviewer identity are never returned publicly. The sanitised `ltsPublishedSegments` collection continues to feed the statewide overlay without comments. Tied vote totals lean toward the higher-stress value so that uncertainty is not hidden.
