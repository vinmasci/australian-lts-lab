...
Raw votes, contributor identity and reasoning are private server-side records under `ltsSegments/{segment}/votes`; publications and reviewer corrections have append-only audit records in `ltsReviewDecisions`, while current results are stored in `ltsApprovals`. Only the sanitised `ltsPublishedSegments` collection feeds `/api/lts-votes?dataset=victoria&approved=1`, so AusBUG clients never receive voter hashes, email addresses, names, notes, reasoning or reviewer identity. Tied vote totals lean toward the higher-stress value so that uncertainty is not hidden.
