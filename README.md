...
Votes do not alter the underlying OSM-derived network. Each signed-in contribution immediately recalculates the community result and updates the separate published override layer. The server verifies the contributor's Firebase ID token, keys their vote to the verified UID and applies account and connection rate limits. The protected `/ltsmap/review` page (also `/review`) remains available to allowlisted reviewers for audits, corrections and removal; account creation remains with the main AusBUG app.
