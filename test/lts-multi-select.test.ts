...
test('Re-selecting a segment removes it and preserves other segments', () => {
  ...
  assert.deepEqual(toggleSegment([a, b], { segment: segment('b') }), [a]);
  assert.deepEqual(toggleSegment([a], { segment: segment('a') }), []);
  assert.deepEqual(toggleSegment(toggleSegment([a, b], a), a), [b, a]);
});

test('Selection cap does not prevent deselecting a segment', () => {
